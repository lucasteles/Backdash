...
)
{
    ...
    ) where TInput : struct
    {
        ...
        var inbox = new ProtocolInbox<TInput>(options, inputSerializer, state, clock, syncManager, checksumStore,
            outbox, networkEventHandler, inputEventQueue, logger);
        ...
        PeerConnection<TInput> connection = new(
            ...
            syncManager, inbox, outbox, checksumStore, inputBuffer);
