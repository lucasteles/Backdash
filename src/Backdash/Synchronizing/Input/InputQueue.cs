...
sealed class InputQueue<TInput> where TInput : unmanaged
{
    ...
    void AddDelayedInputToQueue(GameInput<TInput> input, in Frame frameNumber)
    {
        ...
        // We've been predicting...  See if the inputs we've gotten match what we've been predicting.  If so, don't worry about it.  If not,
        ...
        // If this input is the same frame as the last one requested, and we still haven't found any mis-predicted inputs, we can dump out
        // of prediction mode entirely!  Otherwise, advance the prediction frame count up.
    ...
    Frame AdvanceQueueHead(Frame frame)
    {
        ...
        {
            // This can occur when the frame delay has been increased since the last time we shoved a frame into the system.  We need to replicate the
            // last frame in the queue several times in order to fill the space left.
            logger.Write(LogLevel.Information,
